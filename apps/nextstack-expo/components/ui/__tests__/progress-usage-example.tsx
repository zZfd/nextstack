...
export function AnimatedProgressExample() {
  ...
  return (
    <View className="gap-2 p-4">
      <Text className="text-sm font-medium">Animated Progress: {progress}%</Text>
    ...
    <View className="gap-4 p-4">
      ...
      <View className="gap-2">
        <View className="flex-row items-center justify-between">
          ...
          <Text className="text-sm font-medium">{Math.round((current / max) * 100)}%</Text>
        </View>
        <Progress value={current} max={max} />
      </View>
      <View className="flex-row gap-2">
        <Button onPress={() => setCurrent((prev) => Math.max(0, prev - 1))} className="flex-1">
          <Text>-</Text>
        </Button>
        <Button onPress={() => setCurrent((prev) => Math.min(max, prev + 1))} className="flex-1">
          <Text>+</Text>
        </Button>
      </View>
    </View>
